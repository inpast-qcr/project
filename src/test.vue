...
<!--                     <el-table-column label="序号" prop="xh">
                    ...
                    <label v-if="row.childCount > 0"> 
                        <el-button icon="el-icon-plus" v-if="!row.visible" class="xhButton" @click="searchChildData(row)"></el-button> 
                        <el-button icon="el-icon-minus" v-else-if="row.visible" class="xhButtonjian" @click="searchChildData(row)"></el-button> 
                        {{ row.xh }}
                    </label>
                    <label v-else class="xhButtonText">{{row.xh}}</label>
    ...
    methods: {
        ...
        async searchChildData(){
            for(let i = 0;i<this.tableData.length; i++){
                ...
                // for(let a = 0;a<this.pagination.total;a++){
                //     this.$set(this.tableData[a],'children',res.data.value.list.map((e,index)=>{return{...e,xh: index + 1}}))
                //     console.log(this.tableData[a],a);
                // }
                // this.$set(this.tableData[row.xh-1],'visible',!row.visible)
            }
            console.log(this.tableData,'this.tableData');
/*             this.tableChildData = res.data.value.list.map((item,index)=>{
                    let arrs = {}
                    arrs ={
                        type: item.type == '1' ? '设备' : '耗材',
                        isActive: item.isActive == '1' ? '启用' : '停用',
                ...
                for(var n = 0;n<item.childCount;n++){
                    console.log(tableChildData[n],n+1);
                } */
